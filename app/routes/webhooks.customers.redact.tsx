...
import { webhookCustomerRedactSchema } from "../utils/validation";
import { handleCustomerRedact } from "../utils/gdpr";
...
export const action = async ({ request }: ActionFunctionArgs) => {
  try {
      ...
      if (shop && payload) {
        Promise.resolve().then(async () => {
          await processWebhookSafely(
            ...
            payload,
            async () => {
              const validatedPayload = webhookCustomerRedactSchema.parse(payload);
              await handleCustomerRedact(shop, validatedPayload);
            }
          );
        });
      }
    ...
    if (
      ...
      console.error("CUSTOMERS/REDACT: HMAC validation failed");
